...
public final class SpanUtils {
    ...
    private void updateImage() {
        ...
        int end = mBuilder.length();
        if (imageBitmap != null) {
            mBuilder.setSpan(new CustomImageSpan(mTextView.getContext(), imageBitmap, alignImage), start, end, flag);
        } else if (imageDrawable != null) {
            mBuilder.setSpan(new CustomImageSpan(mTextView.getContext(), imageDrawable, alignImage), start, end, flag);
        } else if (imageUri != null) {
            mBuilder.setSpan(new CustomImageSpan(mTextView.getContext(), imageUri, alignImage), start, end, flag);
        } else if (imageResourceId != -1) {
            mBuilder.setSpan(new CustomImageSpan(mTextView.getContext(), imageResourceId, alignImage), start, end, flag);
        }
    }
...
//            }
    ...
    static class CustomImageSpan extends CustomDynamicDrawableSpan {
        ...
        private CustomImageSpan(final Context context, final Drawable d, final int verticalAlignment) {
            super(verticalAlignment);
            mContext = context;
            mDrawable = d;
            mDrawable.setBounds(
