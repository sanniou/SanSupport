...
package com.sanniou.common.widget.swiper;
...
public class NormalFooter extends AbsSwipeView {
  ...
  @Override
  public void setEnable(boolean canRequest) {
    super.setEnable(canRequest);
    mTvTips.setText(getContext()
        .getString(isEnable() ? R.string.normal_tips_2 : R.string.normal_tips_3));
  }
  ...
  public float scrollOffset(float totalHeight, int offset) {
    //刷新或者执行完成动画时不进行移动
    if (mState == STATE_LOADING || mState == STATE_COMPLETE) {
      return 0;
    }
  ...
  public void endScroll(float totalY) {
    //处于"释放刷新"状态
    if (mState == STATE_PULLING && isEnable()) {
      setLoading();
    } else {
      hideAction();
    }
  ...
  public void pullAction() {
    ...
    mTvTips.setText(getContext()
        .getString(isEnable() ? R.string.pulling_tips_2 : R.string.pulling_tips_3));
  }
 ...
 *//*
  ...
  private void setHeadHeight(float height) {
    ...
    //过滤刷新动作往上至指定高度之后的高度值
    if (!mRefreshToUp || height >= mFinalHeight) {
      mHeadHeight = height;
